public class Solution {
    ...
    private int[] GetLowestIndeciesOnRight(int[] heights){
        ...
        if(stack.Count > 0) {
            while(stack.Count > 0) {
                var poped = stack.Pop();
                result[poped] = heights.Length; // ставим тут +1 индекс от максмимального для удобноого арсчета площади дальше (лайфхак)
            }
        }
        return result;
    }
    ...
    private int[] GetLowestIndeciesOnLeft(int[] heights){
        ...
        if(stack.Count > 0) {
            while(stack.Count > 0) {
                var poped = stack.Pop();
                result[poped] = -1;   // ставим тут -1 индекс для удобноого арсчета площади дальше (лайфхак)
            }
        }
        return result;
    }
